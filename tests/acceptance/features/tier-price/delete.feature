...
Feature: Delete Tier Prices
  ...
  I simply want to put CSV files with tier prices into a directory on the server and they should be deleted

  Rules:
    - Filename starts with "product-import-tier-price_*"
    - The files are available in folder "var/importexport"

Scenario: Delete Tier Prices
  Given attribute sets have been imported
    ...
    And files with tier prices to be deleted are available
    And the tier price deletion process has been started
...
Scenario: Check Tier Price for Drive Shoulder Pack
  Given I am on the homepage
   ...
   Then the response status code should be 200
    And I should not see "Buy 3 for 23,04 € each and save 28%"